 ...
 *
 * Configuration for deploying Next.js to Kubernetes (k3s agent node).
 ...
 *
 * The application node runs as a k3s agent that joins the existing
 * k3s server (monitoring cluster) via the k3s join token.
 */
export interface NextJsK8sConfig {
    ...
    /**
     * SSM parameter prefix of the k3s server (monitoring cluster).
     * Used to discover the server URL and join token.
     * @example '/k8s/development'
     */
    readonly k3sServerSsmPrefix: string;
...
export const NEXTJS_K8S_CONFIGS: Record<Environment, NextJsK8sConfig> = {
    [Environment.DEVELOPMENT]: {
        ...
        k3sServerSsmPrefix: '/k8s/development',
    ...
    [Environment.STAGING]: {
        ...
        nodeLabel: 'role=application',
        nodeTaint: 'role=application:NoSchedule',
        k3sServerSsmPrefix: '/k8s/staging',
    ...
    [Environment.PRODUCTION]: {
        ...
        nodeLabel: 'role=application',
        nodeTaint: 'role=application:NoSchedule',
        k3sServerSsmPrefix: '/k8s/production',
        detailedMonitoring: true,
        useSignals: true,
        signalsTimeoutMinutes: 15,
        rootVolumeSizeGb: 20,
