...
export class K8sComputeStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: K8sComputeStackProps) {
    ...
    NagSuppressions.addResourceSuppressions(accessLogsBucket, [
      {
        id: "AwsSolutions-S1",
        reason:
          "Access logs bucket cannot log to itself — this is the terminal logging destination",
      },
    ]);
        ...
        [
          {
            id: "AwsSolutions-L1",
            reason:
              "BucketDeployment Lambda runtime is managed by CDK singleton — cannot override",
          },
        ],
      );
    } catch {
    ...
    // =====================================================================
    const manifestDeployDoc = new SsmRunCommandDocument(
      this,
      "ManifestDeployDocument",
      {
        documentName: `${namePrefix}-deploy-manifests`,
        description:
          "Deploy k8s monitoring manifests — re-syncs from S3, applies via kubectl",
        parameters: {
          S3Bucket: {
            type: "String",
    ...
    new ssm.StringParameter(this, "ElasticIpParam", {
      ...
      description:
        "k3s Elastic IP address (used by Edge stack as CloudFront origin)",
      tier: ssm.ParameterTier.STANDARD,
    });
    ...
    new cdk.CfnOutput(this, "SsmConnectCommand", {
      ...
      description:
        "SSM Session Manager connect command (replace <instance-id>)",
    });
    ...
    new cdk.CfnOutput(this, "ManifestDeployDocumentName", {
      ...
      description:
        "SSM document name for manifest deployment (use with aws ssm send-command)",
    });
