...
describe('Tagging Guard', () => {
    ...
    it('should not have manual cdk.Tags.of() calls outside tagging-aspect.ts', () => {
        ...
        if (violations.length > 0) {
            ...
            throw new Error(
                `Found ${violations.length} manual tag application(s) outside TaggingAspect.\n` +
                `The TaggingAspect is the sole source of truth for tags.\n` +
                `Remove these and let the aspect handle tagging:\n\n${report}`,
            );
        }
    });
});
