...
import { readStackOutputs, CDK_OUTPUTS_FILE, type StackOutput } from '@repo/script-utils/cdk.js';
import { setOutput, writeSummary } from '@repo/script-utils/github.js';
import logger, { LogLevel } from '@repo/script-utils/logger.js';
import { resolveWorkspacePath } from '@repo/script-utils/paths.js';
...
async function runStackOutputs(): Promise<void> {
  ...
  buildStackSummary(stackName, outputs, deployStatus, environment, region, accountId, deployDuration);
  logger.blank();
...
async function runPipelineSummary(): Promise<void> {
  ...
  const results = await Promise.all(
    project.stacks.map((stack) => verifyStack(cfn, stack, env)),
  );
