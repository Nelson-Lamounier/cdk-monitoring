 ...
 *
 * Usage:
 *   npx tsx deploy-nextjs-secrets.ts \
...
const args = parseArgs(
    ...
    'Deploy Next.js secrets via SSM Automation',
);
...
async function main(): Promise<void> {
    logger.header('Deploy Next.js Secrets');
    ...
    if (!docName) {
        emitAnnotation(
            'error',
            `Next.js secrets doc name not found at ${docParam}`,
            'Next.js Secrets Error',
        );
        logger.fatal(`Next.js secrets doc name not found at ${docParam}`);
    }
    ...
    emitAnnotation(
        'error',
        `Next.js secrets deployment failed: ${failureReason}`,
        'Next.js Secrets Error',
    );
...
main().catch((err: unknown) => {
    ...
    emitAnnotation('error', `Next.js secrets failed: ${message}`, 'Next.js Secrets Error');
    logger.fatal(`Next.js secrets failed: ${message}`);
});
