#!/usr/bin/env npx tsx
/**
 * Deploy Start-Admin Secrets
 *
 * Triggers the SSM Automation document that runs
 * `kubernetes-app/workloads/charts/start-admin/deploy.py` on the
 * control-plane node.  `deploy.py` resolves SSM parameters and
 * creates/updates the `start-admin-secrets` K8s Secret in the
 * `start-admin` namespace.
 *
 * Flow:
 *   1. Resolve control-plane instance ID via EC2 tag lookup
 *   2. Resolve SSM Automation document name from SSM
 *   3. Resolve S3 scripts bucket from SSM
 *   4. Start SSM Automation execution with app=start-admin parameters
 *   5. Poll until terminal state
 *   6. On failure → dump step details and exit 1
 *
 * Usage:
 *   npx tsx deploy-admin-secrets.ts \
...
const args = parseArgs(
    [
        ...
        {
            name: 'timeout',
            description: 'Max seconds to wait for automation (default: 300)',
            hasValue: true,
            default: '300',
        },
        {
    ...
    'Deploy start-admin secrets via SSM Automation',
);
...
const timeoutSeconds = parseInt(args['timeout'] as string, 10) || 300;
 ...
 * @returns The parameter value, or undefined if the parameter is absent.
 */
...
/**
 * Resolve a running instance ID by its `k8s:bootstrap-role` EC2 tag.
 *
 * Uses a live EC2 DescribeInstances call rather than the SSM parameter to
 * avoid stale lookups after ASG instance replacements.
 *
 * @param tagValue - The value of the `k8s:bootstrap-role` tag (e.g. `control-plane`).
 * @returns The instance ID if found, or undefined.
 */
...
/**
 * Entry point — triggers SSM Automation to deploy start-admin secrets.
 *
 * @throws Will call `logger.fatal` (which exits 1) on unrecoverable errors.
 */
async function main(): Promise<void> {
    logger.header('Deploy Start-Admin Secrets');
    ...
    // The consolidated k8s-deploy-secrets document handles all app secrets.
    // Its name is written to SSM by the CDK SecretsAutomationStack.
    const docParam = `${ssmPrefix}/deploy/secrets-doc-name`;
    const docName = await getParam(docParam);
    if (!docName) {
        emitAnnotation(
            'error',
            `Start-Admin secrets doc name not found at ${docParam}`,
            'Start-Admin Secrets Error',
        );
        logger.fatal(`Start-Admin secrets doc name not found at ${docParam}`);
    }
    ...
    // The consolidated document is parameterised by AppName so it knows
    // which deploy.py to run and which K8s namespace to target.
    logger.blank();
    logger.task('Starting SSM Automation for start-admin');
    ...
    // Dump step execution details for diagnostics
    if (lastExecution) {
    ...
    emitAnnotation(
        'error',
        `Start-Admin secrets deployment failed: ${failureReason}`,
        'Start-Admin Secrets Error',
    );
...
/**
 * Builds a markdown summary table for the GitHub Actions job summary.
 *
 * @param status - Human-readable status string (e.g. '✅ Success').
 * @param executionId - SSM Automation execution ID.
...
main().catch((err: unknown) => {
    const message = err instanceof Error ? err.message : String(err);
    emitAnnotation('error', `Start-Admin secrets failed: ${message}`, 'Start-Admin Secrets Error');
    logger.fatal(`Start-Admin secrets failed: ${message}`);
});
