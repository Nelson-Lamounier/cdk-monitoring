 ...
 * @example
 * ```typescript
 * const cleanup = new ResourceCleanupProvider(this, 'Cleanup');
 *
 * const logGroup = new logs.LogGroup(this, 'MyLogs', {
 *     logGroupName: '/aws/states/my-state-machine',
 * });
 *
 * cleanup.addLogGroup('/aws/states/my-state-machine', logGroup);
 * ```
 */
...
/**
 * Inline Python Lambda that deletes orphaned resources.
 *
 ...
 *
 * Creates a single shared Lambda + Provider per stack.
 * Call `addLogGroup()`, `addSsmParameter()`, or `addSnsTopic()`
 * to register resources for cleanup. Each creates a lightweight
 * `CustomResource` that runs before the real CDK resource.
 *
 * ## IAM Permissions
 *
 * The Lambda is granted only the specific delete/describe permissions
 * needed for each resource type registered.
 */
export class ResourceCleanupProvider extends Construct {
    /** The underlying CDK Provider (shared Lambda) */
    private readonly provider: cr.Provider;

    /** The cleanup Lambda function */
    private readonly cleanupFunction: lambda.Function;

    /** Track which IAM permission sets have been added */
    ...
    /**
     * Register a CloudWatch Log Group for pre-emptive cleanup.
     *
     * Creates a `CustomResource` that deletes the orphaned log group
     * (if it exists) before CloudFormation creates the real one.
     *
     * @param logGroupName - Full log group name (e.g. `/aws/vendedlogs/states/...`)
     * @param realResource - The CDK construct that creates the log group.
     *   A dependency is added so the cleanup runs first.
     */
    ...
    /**
     * Register an SSM Parameter for pre-emptive cleanup.
     *
     * @param parameterName - Full parameter path (e.g. `/k8s/development/bootstrap/...`)
     * @param realResource - The CDK construct that creates the parameter.
     */
    ...
    /**
     * Register an SNS Topic for pre-emptive cleanup.
     *
     * @param topicName - The static SNS topic name (not the full ARN).
     *   Used as the construct ID and passed to the Lambda, which
     *   reconstructs the ARN at runtime from the account/region.
     * @param realResource - The CDK construct that creates the topic.
     */
    ...
    /**
     * Lazily add IAM permissions for each resource type (only once).
     */
    ...
    /**
     * Convert a resource name/path to a safe CDK construct ID.
     *
     * Strips leading slashes and replaces non-alphanumeric characters
     * with hyphens, then converts to PascalCase.
     *
     * @example
     * `/aws/vendedlogs/states/k8s-dev-bootstrap-orchestrator`
     *   → `AwsVendedlogsStatesK8sDevBootstrapOrchestrator`
     */
