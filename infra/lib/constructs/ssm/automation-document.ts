 ...
 *   - `bootstrap`: K8s node bootstrap (S3 sync from k8s-bootstrap/boot/steps/)
 ...
 * SSM Automation Document construct.
 *
 * Creates an `AWS::SSM::Document` of type `Automation` (schema 0.3)
 * that orchestrates multi-step EC2 bootstrap or deployment workflows.
 *
 * ## Bootstrap vs Deploy
 *
 * | Category   | S3 Sync Source                    | CW Log Group           |
 * |------------|----------------------------------|------------------------|
 * | bootstrap  | `k8s-bootstrap/boot/steps/`      | `/ssm<prefix>/bootstrap` |
 * | deploy     | `<scriptPath parent>/`           | `/ssm<prefix>/deploy`    |
 *
 * ## Lifecycle
 *
 * Documents support versioning via `updateMethod: 'NewVersion'` — updates
 * create a new version without replacing the resource.
 */
export class SsmAutomationDocument extends Construct {
  ...
  /**
   * Builds Automation document content for K8s node bootstrap.
   *
   * S3 sync: `s3://<bucket>/k8s-bootstrap/boot/steps/` → `/data/k8s-bootstrap/<dir>/`
   * Sources `/etc/profile.d/k8s-env.sh` for CDK-configured env vars.
   *
   * Marker cleanup: removes step-specific marker files so that steps which
   * previously failed or were skipped mid-run are always retried on re-execution.
   * Permanent completion markers (EBS attach, DR restore, kubeadm init) are
   * intentionally preserved — they represent destructive one-time operations.
   */
  ...
  /**
   * Builds Automation document content for app secret deployment.
   *
   * S3 sync: `s3://<bucket>/<scriptDir>/` → `/data/<scriptDir>/`
   * Sets KUBECONFIG for kubectl access.
   */
  ...
  /**
   * Builds the standard parameter block for SSM Automation documents.
   */
