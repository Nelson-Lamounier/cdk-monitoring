 ...
 *
 * Applied to compute stacks to ensure task roles never gain
...
export class EnforceReadOnlyDynamoDbAspect implements cdk.IAspect {
    ...
    private inspectPolicyDocument(
        ...
        const resolved = cdk.Stack.of(node).resolve(doc) as {
            Statement?: Array<{
                Effect?: string;
        ...
        for (const statement of resolved.Statement) {
            ...
            for (const action of actions) {
                ...
                const violation = FORBIDDEN_DYNAMODB_ACTIONS.find(
                    (forbidden) => normalisedAction === forbidden.toLowerCase()
                        || this.matchesWildcard(normalisedAction, forbidden.toLowerCase()),
                );
